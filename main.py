...
async def market_context(
    ...
    if clean_type.lower() == "pokemon" or "pokemon" in clean_type.lower() or POKEMONTCG_API_KEY:
        tcg_anchor = await _tcgplayer_raw_anchor(clean_name, set_info["set_name"] or None)
        anchor_prices = tcg_anchor.get("prices", []) or []
        anchor_currency = tcg_anchor.get("currency", "") or "USD"
        if anchor_prices:

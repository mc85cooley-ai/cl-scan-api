...
@safe_endpoint
async # --- Sealed/Memorabilia rate-limit safe helper (added) ---
import time
try:
...
def _safe_llm_call_memorabilia(call_fn, retry_wait_ms=1100):
    try:
        return call_fn()
        ...
        try:
            ...
            # Degraded but non-null fallback
            return {
                "condition_grade": "Pending",
                "confidence": 0.5,
                "overall_assessment": (
                    "Assessment temporarily limited due to high system load. "
                    "From the available visuals, the item presents reasonably well, "
                    "but a full pass could not be completed. Please retry shortly."
                ),
                "spoken_word": (
                    "Looks like we hit a brief system limit while analysing this item. "
                    "From what we can see, it presents fairly well — try again in a moment "
                    "for a deeper pass."
                ),
